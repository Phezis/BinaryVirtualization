...
6) Смещается вперед на shift элементов типа T в представленной линейной памяти.
7) Смещается назад на shift элементов типа T в представленной линейной памяти.
...
2. Разыменовывают указатель на объект, смещенный на idx элементов относительно текущего положения указателя в виртуальном представлении. Аналогично созданию копии объекта, над которым затем последовательно применяются операторы operator+=(idx) и operator*().
